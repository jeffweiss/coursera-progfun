...
class ListsSuite extends FunSuite {
  ...
  test("sum of mix of positive, negative, and zeros") {
    assert(sum(List(-2, 0, 1)) == -1)
  }
  
  test("max of empty list is NoSuchElementException") {
  ...
  test("max of mix of positive, negative, and zeros") {
    assert(max(List(-23,0,5)) == 5)
  }
}
